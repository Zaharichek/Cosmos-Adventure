...
export function spawn_footprint(player, location){
    ...
    if(player.isSneaking || Math.abs(velocity.x) + Math.abs(velocity.z) < 0.1) return;
    let block_standing_on = player.getBlockStandingOn();
    if(block_standing_on?.typeId !== "cosmos:moon_turf") return;
    let {x, y, z} = location;
    ...
    y = block_standing_on.location.y + 1.01;
